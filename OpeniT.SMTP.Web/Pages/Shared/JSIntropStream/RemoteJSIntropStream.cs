...
{
    internal class RemoteBlazorFileStream : BrowserFileStream
    {
        private readonly IJSRuntime _jsRuntime;
        private readonly string _jsRuntimeIdentifier;
        private readonly object?[]? _jsRuntimeArgs;
        ...
        public RemoteBlazorFileStream(
            IJSRuntime jsRuntime,
            long streamLength,
            RemoteBlazorFileStreamOptions options,
            CancellationToken cancellationToken,
            string jsRuntimeIdentifier,
            params object?[]? jsRuntimeArgs)
            : base (streamLength)
        {
            _jsRuntime = jsRuntime;
            _jsRuntimeIdentifier = jsRuntimeIdentifier;
            _jsRuntimeArgs = jsRuntimeArgs;
        ...
        private async Task FillBuffer(PipeWriter writer, CancellationToken cancellationToken)
        {
            ...
            while (offset < Length)
            {
                ...
                try
                {
                    ...
                    var args = new object?[] { offset, segmentSize }.Concat(_jsRuntimeArgs).ToArray();
                    var bytes = await _jsRuntime.InvokeAsync<byte[]>(
                        _jsRuntimeIdentifier,
